...
from mnemo.app.services.retrieval.list_active import list_active_memories
...
async def retrieve(
    ...
    """
    Run BM25 and vector search in parallel, fuse with RRF, apply token budget.
    When query is empty, return confirmed memories (newest first).
    Set valid_only=False to include retracted facts (with valid_until populated).
    Returns list of (edge_id, fact_string, confidence, valid_at, valid_until, retracted_at, episode_id, score).
    """
    budget = token_budget or settings.default_token_budget
    if not query.strip():
        items = await list_active_memories(db, user_id, valid_only=valid_only, limit=limit)
        return fit(items, budget=budget)
